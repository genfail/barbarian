...
namespace GongSolutions.Shell
{
    ...
    public class ShellView : Control, INotifyPropertyChanged
    {
        ...
        public ShellItem[] SelectedItems
        {
                    ...
                    if (selection.QueryGetData(ref format) == 0)
                    {
                        ...
                        for (int n = 0; n < itemCount; ++n)
                        {
                            ...
                            result[n] = new ShellItem(
                                m_CurrentFolder,
                                (IntPtr)((Int64)storage.unionmember + offset));
                        }
			...
			set
			{
				...
				if (value == null || value.Length == 0)
				{
					foreach (ShellItem item in ShellItem)
					{
						m_ComInterface.SelectItem(Shell32.ILFindLastID(item.Pidl), SVSI.SVSI_DESELECT);
					}	
				}
				else
				{
		...
		/// <summary> Set selected by name </summary>
 		public void Select(string fname)
		{
 			if (m_ComInterface == null)
				return;
           foreach (ShellItem item in ShellItem)
        ...
        ComTypes.IDataObject GetSelectionDataObject()
        {
            ...
            m_ComInterface.GetItemObject(SVGIO.SVGIO_SELECTION, typeof(ComTypes.IDataObject).GUID, out result);

            if (result != IntPtr.Zero)
            {
                ComTypes.IDataObject wrapped = (ComTypes.IDataObject)Marshal.GetTypedObjectForIUnknown(result, typeof(ComTypes.IDataObject));
                return wrapped;
            }
            else
            {
